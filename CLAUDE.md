...
SDSS Datathon 2026 — "Airfare Markets Under Pressure": analyze domestic U.S. airfare markets to uncover structural drivers of ticket affordability across competition, hub dominance, route demand, and distance. The project has two components:

1. **React/TypeScript web app** (`frontend/`) — interactive D3.js map visualization
2. **Jupyter notebooks** (`data_case/Airline Tickets/`, `dataset/`) — EDA and modeling
...
# Install (use --legacy-peer-deps due to React 19 peer conflicts)
...
jupyter lab              # Launch JupyterLab for notebooks
```

## Frontend Architecture

All data is pre-processed into static JSON — no backend API.

**Component hierarchy:**
- `App.tsx` — scroll-snap landing page + map section
- `MapContainer.tsx` — holds `year`/`quarter` state (2022–2025), filters route data, runs simulation playback (1000ms/step)
- `USMap.tsx` — D3 Albers USA projection, TopoJSON state paths, airport dots, Bézier flight arc paths, slide-out city info panel
- `TimeFilter.tsx` — year navigation + Q1–Q4 quarter selection

**Data files in `frontend/src/data/`:**
- `airport-coords.json` — `{city, lat, lon}` used for D3 projection
- `routes.json` — flight routes with fare/competition metrics
- `states-10m.json` — TopoJSON US state boundaries

Available time periods: 2022–2024 all quarters, 2025 Q1–Q2 only.

D3 and TopoJSON type declarations are in `frontend/src/vite-env.d.ts` (added manually — no `@types` packages for these).
